...
#mnist = tf.keras.datasets.mnist.read_data_sets('MNIST', one_hot=True)
# 加载MNIST数据集
...
batch_size = 100
# 一共有多少个批次
n_batch = num_train_samples // batch_size
...
x = tf.compat.v1.placeholder(tf.float32, [None, 784], name="x_input")     # placeholder(dtype, shape=None, name=None)
...
# 初始化第一个全连接层的权值
W_fc1 = weight_variable([7*7*64, 1024/2])     # 上一层有7*7*64个神经元，全连接层有1024个神经元
...
# 初始化第二个全连接层的权值
W_fc2 = weight_variable([1024/2, 10])     # 上一层有7*7*64个神经元，全连接层有1024个神经元
